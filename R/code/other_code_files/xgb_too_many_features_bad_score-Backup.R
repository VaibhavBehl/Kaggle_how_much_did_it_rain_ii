...
predictions_xgb <- function(trData, teData) {
  ...
  tr <- tr_raw[, .(
    ...
    meanRefComp = mean(valid_time*RefComposite,na.rm=T),
    meanRefComp50p = mean(valid_time*RefComposite_5x5_50th,na.rm=T),
    meanRefComp90p = mean(valid_time*RefComposite_5x5_90th,na.rm=T),
    meanRefCompAll = mean(valid_time*rowMeans(cbind(RefComposite,RefComposite_5x5_10th,RefComposite_5x5_50th,RefComposite_5x5_90th), na.rm=T),na.rm = T),
  ...
  #tr <- rbindlist(lapply("7030/tr_ensemble7030_many_feat.csv", fread, sep=","))
  ...
  #xgbModel = xgb.load('7030/xgbModel.ensemble7030_many_feat.save')
  #xgbModel = xgb.model.dt.tree(feature_names = xFeatures, filename_dump = '7030/xgbModel.ensemble7030_many_feat.dump')
  ...
  ##re-run after this after adding tr feat into it<<<<<<<<<<<<<<<<
  ...
  #te <- rbindlist(lapply("7030/te_ensemble7030_many_feat.csv", fread, sep=","))
  #write.csv(te,paste("te_ALL_many_feat.csv"),row.names=F)
